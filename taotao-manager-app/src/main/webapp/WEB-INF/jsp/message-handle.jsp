...
<table class="easyui-datagrid" id="messageList" title="商品列表" 
       data-options="singleSelect:false,collapsible:true,pagination:true,url:'/message/dead/list',method:'get',pageSize:30,toolbar:toolbar">
    <thead>
        <tr>
    ...
    var toolbar = ['-',{
        ...
        handler:function(){
        	...
        	$.messager.confirm('确认','重新确认并发送消息？',function(r){
        	    if (r){
        	    	var params = {"ids":ids};
                	$.post("/message/dead/handler",params, function(data){
            			if(data.status == 200){
